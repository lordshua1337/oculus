...
export default function ClientDetailError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error('[Client Detail Error]', error.message, error.digest)
  }, [error])
  ...
  return (
    ...
    >
      ...
      >
        {error.message || 'Failed to load client profile. Please try again.'}
      </p>
      <div style={{ display: 'flex', gap: 12 }}>
        <OculusButton variant="primary" onClick={reset}>
          Try Again
        </OculusButton>
        <OculusButton variant="outline" onClick={() => { window.location.href = '/dashboard/crm' }}>
          Back to CRM
        </OculusButton>
      </div>
    </div>
